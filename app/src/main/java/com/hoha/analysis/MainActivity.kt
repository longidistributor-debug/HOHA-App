...
class MainActivity : Activity() {
    ...
    private var selectedTicker="FCM:EURUSD"
    ...
    override fun onCreate(savedInstanceState:Bundle?){
        ...
        selectedTicker=prefs.getString("selected_ticker","FCM:EURUSD").orEmpty().ifBlank{"FCM:EURUSD"}
        if(selectedTicker.startsWith("FX:")) selectedTicker="FCM:${selectedTicker.substringAfter(':')}"
    ...
    private fun buildUi():View{
        ...
        titles.addView(label("HOHA",28f,true)); titles.addView(label("HAMAD ANALYSIS",12f,true,Color.LTGRAY)); titles.addView(label("HOHA v7 • REST CHART",11f,true,Color.WHITE))
        ...
        socketKey=input("FCS Socket Key (optional)",prefs.getString("socket_key","").orEmpty())
        ...
        keyCard.addView(Button(this).apply{text="SAVE KEYS + LOAD CHART";setTextColor(Color.BLACK);background=rounded(Color.WHITE,dp(12).toFloat());setOnClickListener{saveKeys();resolveAndLoadDefault()}},LinearLayout.LayoutParams(-1,dp(50)).apply{topMargin=dp(10)})
        ...
        val tf=Spinner(this).apply{adapter=ArrayAdapter(this@MainActivity,android.R.layout.simple_spinner_dropdown_item,periods);setSelection(periods.indexOf(selectedPeriod).coerceAtLeast(0))}
        marketRow.addView(tf,LinearLayout.LayoutParams(dp(92),dp(44)))
        marketRow.addView(Button(this).apply{text="INSTRUMENTS";setTextColor(Color.BLACK);background=rounded(Color.WHITE,dp(10).toFloat());setOnClickListener{openBrowser()}},LinearLayout.LayoutParams(dp(128),dp(44)))
        chartCard.addView(marketRow)
        chart=WebView(this).apply{settings.javaScriptEnabled=true;settings.domStorageEnabled=true;setBackgroundColor(Color.BLACK);webViewClient=object:WebViewClient(){override fun onPageFinished(view:WebView?,url:String?){chartReady=true;if(apiKey.text.toString().trim().isNotBlank())resolveAndLoadDefault()}};loadUrl("file:///android_asset/chart.html")}
        chartCard.addView(chart,LinearLayout.LayoutParams(-1,dp(430)).apply{topMargin=dp(6)})
        chartCard.addView(label("REST candle chart: symbol search is handled by HOHA, not the broken chart-library search. Select a symbol with INSTRUMENTS, then change timeframe here.",10f,false,Color.GRAY),LinearLayout.LayoutParams(-1,-2).apply{topMargin=dp(8)})
        root.addView(chartCard)
        tf.onItemSelectedListener=object:AdapterView.OnItemSelectedListener{
            override fun onItemSelected(p:AdapterView<*>?,v:View?,pos:Int,id:Long){val np=periods[pos];if(np!=selectedPeriod){selectedPeriod=np;prefs.edit().putString("selected_period",np).apply();marketText.text="$selectedTicker • $selectedPeriod";loadChartData()}}
        ...
        resultText=label("READY\nLoad chart or choose an instrument, then run analysis.",13f,false,Color.WHITE).apply{setPadding(dp(14),dp(14),dp(14),dp(14));background=rounded(Color.rgb(16,16,16),dp(12).toFloat(),Color.rgb(55,55,55))}
        action.addView(resultText,LinearLayout.LayoutParams(-1,-2).apply{topMargin=dp(12)}); root.addView(action)

        root.addView(sectionTitle("FLOATING MODE")); val fcard=card(); fcard.addView(label("The floating H bubble uses the same selected instrument and timeframe.",12f,false,Color.LTGRAY)); fcard.addView(Button(this).apply{text="ENABLE HOHA FLOAT";setTextColor(Color.WHITE);background=rounded(Color.rgb(30,30,30),dp(14).toFloat(),Color.rgb(90,90,90));setOnClickListener{enableFloat()}},LinearLayout.LayoutParams(-1,dp(54)).apply{topMargin=dp(12)}); root.addView(fcard)
    ...
    private fun openBrowser(){
        ...
        val cats=arrayOf("Forex","Commodity","Crypto","Futures","DEX","Stock","Fund","Index","All")
        val spinner=Spinner(this).apply{adapter=ArrayAdapter(this@MainActivity,android.R.layout.simple_spinner_dropdown_item,cats)}
        val search=input("Search e.g. XAUUSD, EURUSD, BTC, AAPL","")
        val go=Button(this).apply{text="SEARCH / SHOW LIST"}
        val status=label("Choose category or search.",11f,false,Color.DKGRAY)
        ...
        val dialog=AlertDialog.Builder(this).setTitle("HOHA Instruments").setView(box).setNegativeButton("Close",null).create()
        ...
        fun load(){
            ...
            thread{
                try{
                    val(res,calls)=MarketClient.browse(key,cat,q)
                    runOnUiThread{
                        addUsage(calls); usageText.text="Calls: ${usage()}/500"; items=res
                        list.adapter=ArrayAdapter(this,android.R.layout.simple_list_item_1,res.map{"${it.ticker}   ${it.name}"})
                        status.text=if(res.isEmpty())"No result from FCS. Try another category/search." else "${res.size} supported instruments"
                    }
                }catch(e:Exception){runOnUiThread{status.text="Load failed: ${e.message}"}}
            }
        }
        go.setOnClickListener{load()}
        spinner.onItemSelectedListener=object:AdapterView.OnItemSelectedListener{override fun onItemSelected(p:AdapterView<*>?,v:View?,pos:Int,id:Long){load()};override fun onNothingSelected(p:AdapterView<*>?){}}
        list.setOnItemClickListener{_,_,pos,_->items.getOrNull(pos)?.let{selectInstrument(it);dialog.dismiss()}}
        dialog.show()
    }

    private fun resolveAndLoadDefault(){
        val key=apiKey.text.toString().trim(); if(key.isBlank()||!chartReady)return
        if(selectedTicker.contains(":")){loadChartData();return}
        thread{
            try{
                val(res,calls)=MarketClient.browse(key,selectedType,selectedSymbol)
                val exact=res.firstOrNull{it.symbol.equals(selectedSymbol,true)} ?: res.firstOrNull()
                runOnUiThread{addUsage(calls);usageText.text="Calls: ${usage()}/500";if(exact!=null)selectInstrument(exact)else loadChartData()}
            }catch(_:Exception){runOnUiThread{loadChartData()}}
        }
    }
    ...
    private fun loadChartData(){
        ...
        val key=apiKey.text.toString().trim(); if(key.isBlank())return
        chart.evaluateJavascript("showMessage(${JSONObject.quote("Loading $selectedTicker • $selectedPeriod...")})",null)
        thread{
            try{
                val(data,credits)=FcsClient.history(key,selectedTicker,selectedSymbol,selectedType,selectedPeriod,180)
                val arr=JSONArray(); data.forEach{arr.put(JSONObject().put("t",it.t).put("o",it.o).put("h",it.h).put("l",it.l).put("c",it.c).put("v",it.v))}
                runOnUiThread{
                    addUsage(credits); usageText.text="Calls: ${usage()}/500"; marketText.text="$selectedTicker • $selectedPeriod"
                    chart.evaluateJavascript("renderCandles(${JSONObject.quote(arr.toString())},${JSONObject.quote(selectedTicker)},${JSONObject.quote(selectedPeriod)})",null)
                    resultText.text="CHART READY: $selectedTicker • $selectedPeriod\n${data.size} candles loaded."
                }
            }catch(e:Exception){runOnUiThread{chart.evaluateJavascript("showMessage(${JSONObject.quote("No candle data: ${e.message}")})",null);resultText.text="CHART LOAD FAILED\n${e.message}"}}
        }
    }
    ...
    private fun analyze(){
        ...
        saveKeys(); busy=true; resultText.text="ANALYZING $selectedTicker • $selectedPeriod\nScoring weighted confirmations..."
        thread{
            try{
                val(data,credits)=FcsClient.history(key,selectedTicker,selectedSymbol,selectedType,selectedPeriod,180); addUsage(credits); val signal=AnalysisEngine.analyze(data)
