...
class MainActivity : Activity() {
    ...
    private fun buildUi(): View {
        ...
        titles.addView(label("Multi-Market Weighted Analysis", 11f, false, Color.GRAY))
        ...
        keyCard.addView(Button(this).apply {
            text = "SAVE KEYS + LOAD CHART"
            setTextColor(Color.BLACK)
            background = rounded(Color.WHITE, dp(12).toFloat())
            setOnClickListener { saveKeys(); initChart() }
        ...
        chart = WebView(this).apply {
            ...
            webViewClient = object : WebViewClient() {
                override fun onPageFinished(view: WebView?, url: String?) { chartReady = true; initChart() }
            }
            loadUrl("file:///android_asset/chart.html")
        }
        chartCard.addView(chart, LinearLayout.LayoutParams(-1, dp(470)).apply { topMargin = dp(6) })
        chartCard.addView(label("Tap INSTRUMENTS for the real FCS list/search. Blank search shows symbols; categories include Forex, Gold/Commodities, Crypto, Futures, DEX, Stocks, Funds and Indices.", 10f, false, Color.GRAY), LinearLayout.LayoutParams(-1, -2).apply { topMargin = dp(8) })
        ...
        top.addView(label("REST quota", 11f, false, Color.GRAY))
        action.addView(top)
        ...
        resultText = label("READY\nSelect an instrument inside the chart card, then run analysis.", 13f, false, Color.WHITE).apply {
        ...
        val fcard = card()
        fcard.addView(label("Use the draggable H bubble over MT5. Last selected instrument is shared with floating mode.", 12f, false, Color.LTGRAY))
    ...
    private fun openBrowser() {
        ...
        box.addView(search, LinearLayout.LayoutParams(-1, dp(52)).apply { topMargin = dp(6) })
        box.addView(go, LinearLayout.LayoutParams(-1, dp(48)).apply { topMargin = dp(6) })
        box.addView(status)
        box.addView(list, LinearLayout.LayoutParams(-1, dp(430)))
        ...
        fun load() {
            val cat = cats[spinner.selectedItemPosition].lowercase()
            val q = search.text.toString().trim()
            status.text = "Loading ${cats[spinner.selectedItemPosition]}..."
            thread {
                try {
                    ...
                    runOnUiThread {
                        ...
                        status.text = if (res.isEmpty()) "No instruments returned for this category/search." else "${res.size} instruments shown. Search to find more."
                    }
                } catch (e: Exception) {
    ...
    inner class ChartBridge {
        ...
        fun onPeriodChanged(period: String) {
            ...
            runOnUiThread { marketText.text = "$selectedTicker • $selectedPeriod" }
        }
    }

    private fun saveKeys() {
        prefs.edit().putString("api_key", apiKey.text.toString().trim()).putString("socket_key", socketKey.text.toString().trim()).apply()
        Toast.makeText(this, "Keys saved locally", Toast.LENGTH_SHORT).show()
    }
    ...
    private fun initChart() {
        ...
        val socket = socketKey.text.toString().trim()
        chart.evaluateJavascript("initLiveChart(${JSONObject.quote(key)},${JSONObject.quote(selectedTicker)},${JSONObject.quote(selectedPeriod)},${JSONObject.quote(socket)})", null)
    }
    ...
    private fun analyze() {
        ...
        saveKeys()
        busy = true
        resultText.text = "ANALYZING $selectedTicker • $selectedPeriod\nScoring weighted confirmations..."
        thread {
            try {
                ...
                Handler(Looper.getMainLooper()).post { resultText.text = "REQUEST FAILED\n${e.message ?: "Unknown error"}"; busy = false }
            }
        }
    }
    ...
    private fun card() = LinearLayout(this).apply {
        orientation = LinearLayout.VERTICAL; setPadding(dp(14), dp(14), dp(14), dp(14)); background = rounded(Color.rgb(10,10,10), dp(16).toFloat(), Color.rgb(45,45,45))
    }
    ...
    private fun usage(): Int {
        val m = monthKey()
        if (prefs.getString("usage_month", "") != m) prefs.edit().putString("usage_month", m).putInt("usage", 0).apply()
