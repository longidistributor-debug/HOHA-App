...
class OverlayService : Service() {
    ...
    override fun onCreate() {
        super.onCreate()
        ticker = prefs.getString("selected_ticker", "FX:EURUSD").orEmpty().ifBlank { "FX:EURUSD" }
        symbol = prefs.getString("selected_symbol", "EURUSD").orEmpty().ifBlank { "EURUSD" }
        assetType = prefs.getString("selected_type", "forex").orEmpty().ifBlank { "forex" }
    ...
    private fun foreground() {
        ...
        val n = (if (Build.VERSION.SDK_INT >= 26) Notification.Builder(this, id) else Notification.Builder(this))
            ...
            .setContentText("Floating market analysis active")
    ...
    private fun showPanel() {
        ticker = prefs.getString("selected_ticker", ticker).orEmpty().ifBlank { ticker }
        symbol = prefs.getString("selected_symbol", symbol).orEmpty().ifBlank { symbol }
        assetType = prefs.getString("selected_type", assetType).orEmpty().ifBlank { assetType }
        period = prefs.getString("selected_period", period).orEmpty().ifBlank { period }
        ...
        title.addView(text("HOHA • $ticker", 15f, true), LinearLayout.LayoutParams(0, dp(42), 1f))
        ...
        status = text("Using main-app selected instrument: $ticker • $period", 11f)
    ...
    private fun initChart() {
        ...
        val socket = prefs.getString("socket_key", "").orEmpty()
        chart?.evaluateJavascript("initLiveChart(${JSONObject.quote(key)},${JSONObject.quote(ticker)},${JSONObject.quote(period)},${JSONObject.quote(socket)})", null)
    }

    private fun hidePanel() {
        panel?.let { runCatching { wm.removeView(it) } }
        panel = null; chart = null; status = null; counter = null; chartReady = false
    }
    ...
    private fun analyze() {
        ...
        busy = true
        status?.text = "Scoring $ticker $period..."
        thread {
            try {
                ...
                Handler(Looper.getMainLooper()).post { status?.text = "Request failed: ${e.message}"; busy = false }
            }
        }
    }
