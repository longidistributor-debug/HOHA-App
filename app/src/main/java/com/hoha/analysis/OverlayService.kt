...
class OverlayService : Service() {
    ...
    override fun onCreate(){super.onCreate();ticker=prefs.getString("selected_ticker","FX:EURUSD").orEmpty().ifBlank{"FX:EURUSD"};symbol=prefs.getString("selected_symbol","EURUSD").orEmpty().ifBlank{"EURUSD"};assetType=prefs.getString("selected_type","forex").orEmpty().ifBlank{"forex"};period=prefs.getString("selected_period","15m").orEmpty().ifBlank{"15m"};wm=getSystemService(WINDOW_SERVICE) as WindowManager;foreground();createBubble()}
    ...
    private fun showPanel(){ticker=prefs.getString("selected_ticker",ticker).orEmpty().ifBlank{ticker};symbol=prefs.getString("selected_symbol",symbol).orEmpty().ifBlank{symbol};assetType=prefs.getString("selected_type",assetType).orEmpty().ifBlank{assetType};period=prefs.getString("selected_period",period).orEmpty().ifBlank{period};val root=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(dp(12),dp(10),dp(12),dp(12));background=GradientDrawable().apply{setColor(Color.rgb(8,8,8));cornerRadius=dp(18).toFloat();setStroke(dp(1),Color.GRAY)}};panel=root;val title=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.CENTER_VERTICAL};title.addView(text("HOHA • $ticker",15f,true),LinearLayout.LayoutParams(0,dp(42),1f));title.addView(Button(this).apply{text="—";setOnClickListener{hidePanel()}},LinearLayout.LayoutParams(dp(48),dp(40)));root.addView(title);chart=WebView(this).apply{settings.javaScriptEnabled=true;settings.domStorageEnabled=true;setBackgroundColor(Color.BLACK);webViewClient=object:WebViewClient(){override fun onPageFinished(view:WebView?,url:String?){chartReady=true;initChart()}};loadUrl("file:///android_asset/chart.html")};root.addView(chart,LinearLayout.LayoutParams(-1,dp(320)));counter=text("Calls: ${usage()}/500",11f,true);root.addView(counter);root.addView(Button(this).apply{text="RUN WEIGHTED ANALYSIS";setTextColor(Color.BLACK);setBackgroundColor(Color.WHITE);setOnClickListener{analyze()}},LinearLayout.LayoutParams(-1,dp(50)));status=text("Using main-app selected instrument: $ticker • $period",11f);status?.setPadding(0,dp(8),0,0);root.addView(status);val p=WindowManager.LayoutParams(dp(360),dp(560),type(),WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE,PixelFormat.TRANSLUCENT).apply{gravity=Gravity.TOP or Gravity.END;x=dp(8);y=dp(55)};wm.addView(root,p)}
    private fun initChart(){if(!chartReady)return;val key=prefs.getString("api_key","").orEmpty();if(key.isBlank())return;val socket=prefs.getString("socket_key","").orEmpty();chart?.evaluateJavascript("initLiveChart(${JSONObject.quote(key)},${JSONObject.quote(ticker)},${JSONObject.quote(period)},${JSONObject.quote(socket)})",null)}
